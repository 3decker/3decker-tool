...
#ifdef _WIN32
// GetFileSize() returns only a 32-bit DWORD and silently wraps for files
// past 4GB when the high-order-DWORD output pointer is NULL (as used below) -
// real Blu-ray-scale inputs (tens of GB) then appear only a few GB long, so
// decoding stops early with no error. GetFileSizeEx() returns the true
// 64-bit size and must be used for any input that can exceed 4GB.
static uint64_t win32_file_size64(HANDLE f) {
	LARGE_INTEGER li;
	return GetFileSizeEx(f, &li) ? (uint64_t)li.QuadPart : 0;
}
#endif
...
static int decode_file(const char *name0)
{
	...
	conf[0] = conf[1] = NULL;
		...
		const uint8_t *end0 = v0 != NULL ? v0 + win32_file_size64(f0) : NULL;
		const uint8_t *end1 = v1 != NULL ? v1 + win32_file_size64(f1) : NULL;
	#else
