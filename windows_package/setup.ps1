...
if (Test-Path (Join-Path $nunifDir ".git")) {
    # Real bug, confirmed from an actual user report: this used to just run
    # `git pull --ff` on WHATEVER branch happened to be checked out, with no
    # check at all. If that folder's nunif\ was ever switched to a different
    # branch -- e.g. by following 3DECKER_INSTALL.md's own "revert to plain
    # iw3" instructions (`git checkout master`, which on this repo tracks
    # nagadomi/nunif's own upstream, not this fork) -- re-running setup.ps1
    # would silently keep pulling more of THAT branch forever. No warning, no
    # error: it just looks like "the GUI is old" when it's actually the wrong
    # codebase entirely (plain upstream nunif, not 3DECKER). Now checks the
    # branch first and fails loudly instead of guessing or silently pulling
    # the wrong thing.
    $currentBranch = (& $gitExe -C $nunifDir rev-parse --abbrev-ref HEAD 2>$null)
    if ($LASTEXITCODE -ne 0) { $currentBranch = "(unknown)" } else { $currentBranch = $currentBranch.Trim() }
    if ($currentBranch -ne "my-customizations") {
        throw ("Found '$nunifDir' as a real git repository, but it is currently on branch " +
            "'$currentBranch', not 'my-customizations' -- pulling here would silently update " +
            "whatever '$currentBranch' actually points to, which may not be 3DECKER's own code " +
            "at all (e.g. plain upstream nunif, if this folder was ever switched to 'master'). " +
            "Fix: open a terminal in '$nunifDir' and run 'git checkout my-customizations' (if " +
            "that fails because the branch doesn't exist locally yet, first run 'git remote add " +
            "backup https://github.com/3decker/3decker-tool.git' -- or skip that if it already " +
            "exists -- then 'git fetch backup' and 'git checkout -b my-customizations " +
            "backup/my-customizations'), then re-run this setup script.")
    }
    Write-Host "  Already present at $nunifDir on branch 'my-customizations' -- pulling latest instead of cloning."
...
# ADR-137: the actual YAML content lives in one place,
# iw3/inpaint_utils.py's OPTIONAL_INPAINT_MODELS_YAML -- update-3decker.bat
# calls the exact same Python function, so existing installs that set up
# before this feature shipped also get it via a later "Install Update Now",
# not only fresh installs.
Push-Location $nunifDir
try {
    & $pythonExe -m iw3.register_optional_inpaint_models
    if ($LASTEXITCODE -ne 0) { throw "iw3.register_optional_inpaint_models failed with exit code $LASTEXITCODE" }
} finally {
    Pop-Location
}
