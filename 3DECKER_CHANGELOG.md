...
**Fixed: a black command-window kept flashing open and closed throughout the "Convert to MVC" step.** Reported directly: "there was flashing the cmd window back and forth like opening and closing... the work got done, but that was distracting." This step runs many small background tasks in a row (pulling out each audio/subtitle track, preparing the video, the actual 3D encode, then combining everything) — each one was popping open its own brief window because that particular step runs as its own separate background program, which had never been told to stay hidden the way the rest of this program already is. It's now told to stay hidden too, so the whole step runs quietly in the background like everything else. (The step still takes a while — that part is a real limit of the free 3D-video encoder involved, not something this fix changes.)
