...
**New: "Write a Log File for This Job" now also records the actual settings used, not just file paths and progress.** Pointed out directly: the log file this option creates didn't say anything about which depth model, resolution, 3D strength, etc. actually made the file — just the input/output paths and a progress readout. The log now includes a plain settings list (one setting per line) right at the top, reusing this project's own already-proven settings tracker so it can never disagree with what's actually embedded in the file itself.

## Update — September 25, 2026

**Corrected: every "real MVC" option's tooltip actually plays as ordinary 2D in any normal player — they previously said they couldn't.** Real feedback from an end user pointed this out: an MVC file's base picture is a completely standard video track on its own, so any regular player just shows it as normal 2D, ignoring the extra 3D data it doesn't understand — the same reason a plain 2D Blu-ray player can still play a real 3D disc. Verified this directly rather than just taking it on faith: made a real MVC file from a real 3D Blu-ray disc, decoded 30 real seconds of it with a completely ordinary player engine, and confirmed a clean, correct picture the whole way through. Every tooltip touching a real-MVC output now says this clearly and names real devices that play the actual 3D side of it well, based on that same user's own experience: Kodi and its variants (OSMC, LibreELEC, including the newer Raspberry Pi 4/5 build), several Android streaming boxes, and some real hardware 3D Blu-ray/UHD players. Covers "3D Blu-ray Import"'s Lossless MVC .mkv, the main tab's "Convert to 3D Blu-ray MVC after conversion," and the standalone "SBS to 3D Blu-ray MVC" tool. The same end user added a real follow-up worth passing on: this single-file 2D+3D convenience is genuinely useful even for a homemade AI conversion (not just a real studio disc rip) — you still avoid keeping separate 2D and 3D copies of a movie you convert yourself, even though a homemade conversion obviously isn't the same as a genuine disc's own original quality. That honest distinction is now stated directly in the relevant tooltips too.

**New: "Write a Log File" now also covers Standalone Tools, not just the main conversion.** Reported directly: a log file didn't get created for a job that had gone through the main conversion, then RIFE, then Restore Audio & Subtitles. Turned out "Write a Log File for This Job" (Post-Processing section) was only ever built to cover the main conversion tab's own run — RIFE, Restore Audio & Subtitles, and every other tool on the Standalone Tools tab each run as their own separate program, and were never covered by any log option at all, no matter what was checked. There's now a matching "Write a Log File for Standalone Tool Runs" checkbox right next to the shared output box at the bottom of the Standalone Tools tab — turn it on and whichever tool you run there saves its own real output (including the exact reason for a failure) into a text file next to that tool's own output file, the same "_log.txt" naming the main conversion's own log option already uses. Off by default. Covers every Standalone Tool that produces its own file: Sharpen, 3D Blu-ray Import, Retroactive HDR/DV Reinjection, Add Subtitle Track, Add Audio Track, Restore All Audio Tracks, RIFE Interpolation, SBS to 3D Blu-ray MVC, Convert HDR/DV to SDR, Upscale with waifu2x, and Retroactively Tag MKV as 3D.
