...
class MainFrame(wx.Frame):
    ...
    def initialize_component(self):
        ...
        self.cbo_divergence.SetToolTip(
            T("Also called Divergence. The master strength of the whole 3D effect — how far things shift "
              "between the left/right eye. Higher = more dramatic depth, but more edge artifacts. Lower = "
              "subtler, cleaner. Recommended: 2.0-3.0 for most desktop/game content."))
        ...
        self.cbo_convergence_mode.SetToolTip(
            T("How the \"screen depth\" is chosen. constant: fixed position, set with the value box — "
              "recommended for real-time desktop/game capture, since it costs no extra per-frame "
              "processing and gives the steadiest result. sod_v1/face_detect: AI automatically re-picks "
              "the focus point every frame — adds real per-frame processing cost, which matters more here "
              "than in an offline video conversion since it directly affects live latency/frame rate."))
        ...
        self.cbo_convergence.SetToolTip(
            T("Also called Convergence Plane. Where the \"screen depth\" sits, from 0 (everything pops "
              "out toward you) to 1 (everything sits behind the screen). Recommended: 0.5 as a balanced "
              "starting point."))
        ...
        self.cbo_synthetic_view.SetToolTip(
            T("Which eye view gets newly generated by AI. \"both\" generates both eyes fresh (most "
              "natural). \"right\"/\"left\" keep the original captured frame as one eye and only "
              "synthesize the other — half the per-frame work, meaningfully faster for real-time capture "
              "at the cost of a slightly less balanced look."))
        ...
        self.cbo_method.SetToolTip(
            T("Which technique builds the second eye view. No *_inpaint variants here (unlike the file-"
              "based iw3-gui) since real-time capture needs every frame fast — inpainting's extra AI pass "
              "isn't practical at live frame rates. row_flow_v3/row_flow_v3_sym: fast, good default. "
              "mlbw_l2/l4: higher quality layered warp, more GPU cost; mlbw_l2s is a lighter/faster "
              "version of mlbw_l2. forward_fill/monobw: simplest, fastest, roughest edges — useful if you "
              "need every last bit of frame rate. Recommended: row_flow_v3_sym for a good quality/speed "
              "balance at real-time speeds."))
        ...
        self.chk_small_model_only.SetToolTip(
            T("What it's for: filters the Depth Model dropdown down to only the smaller/faster model "
              "variants (_S), hiding the larger/slower ones (_B/_L).\n"
              "Why it matters here specifically: unlike offline video conversion, this runs live every "
              "frame — a large depth model that would be fine for a one-time file conversion can drop "
              "your real-time frame rate noticeably.\n"
              "Recommended: on for real-time desktop/game capture; turn off only if you have a very "
              "powerful GPU and want to try a larger model's quality at a real frame-rate cost."))
        ...
        self.cbo_depth_model.SetToolTip(
            T("Which AI model estimates depth from each captured frame. Bigger models (_B/_L suffix) give "
              "better quality but cost real frame rate in this real-time context — unlike a one-time file "
              "conversion where extra time barely matters. Recommended: a small (_S) model for smooth "
              "real-time capture; only go bigger if your GPU has headroom to spare and you've confirmed "
        ...
        self.cbo_resolution.SetToolTip(
            T("How much detail the depth model works with internally. Higher = finer depth detail but "
              "more GPU time per frame — directly costs real-time frame rate here. Recommended: Default "
              "for smooth real-time capture."))
        ...
        self.cbo_foreground_scale.SetToolTip(
            T("Reshapes the depth curve (-3 to 3) as a trade-off, not a free improvement: positive values "
              "sharpen/separate the foreground while flattening the background; negative values do the "
              "reverse. 0 = natural, unexaggerated depth. Recommended: 0 to start."))
        ...
        self.lbl_edge_dilation.SetToolTip(
            T("Smooths the depth map at object silhouettes before the 3D shift is applied, reducing "
              "halo/distortion at foreground edges. This is a FIXED amount of smoothing that doesn't "
              "scale itself with 3D Strength — raise it if you raise 3D Strength, to keep the same level "
              "of edge protection."))
        self.cbo_edge_dilation.SetToolTip(
            T("X value: horizontal edge smoothing strength. Recommended: 2 as a starting point, more if "
              "you raise 3D Strength."))
        self.cbo_edge_dilation_y.SetToolTip(
            T("Y value: additional vertical edge smoothing, on top of X (blank = same as X, fully "
              "symmetric — the default here). Recommended: leave blank unless you notice vertical edges "
              "specifically need more/less smoothing than horizontal ones."))
        ...
        self.chk_ema_normalize.SetToolTip(
            T("Video only (experimental). Smooths the depth map's overall near/far scale across "
              "consecutive captured frames, so the whole image's depth intensity doesn't visibly "
              "pulse/breathe from the AI model's own frame-to-frame noise. Recommended: on."))
        ...
        self.cbo_ema_decay.SetToolTip(
            T("Decay Rate: how much past frames are weighted vs. the newest frame. Higher (0.9-0.99) = "
              "steadier but slower to follow a real change (e.g. switching to a very different game "
              "scene). Lower (0.5-0.75) = reacts faster, more exposed to per-frame noise. Recommended: "
              "0.9 as a starting point for live/interactive content."))
        ...
        self.chk_preserve_screen_border.SetToolTip(
            T("Tapers the 3D shift down to zero right at the frame edges, so an object with strong 3D pop "
              "doesn't create a visual conflict with the frame border cutting it off. Very slightly "
              "reduces 3D strength right at the extreme edges only. Recommended: on."))
        ...
        self.cbo_stereo_format.SetToolTip(
            T("The layout streamed/displayed. Half SBS/TB (side-by-side/top-bottom, squeezed to the "
              "original frame size) are the standard, most compatible formats for 3D TVs/headsets over a "
              "live stream. Full SBS/TB keep both eyes at full resolution (roughly double the streaming "
              "bandwidth). Cross Eyed needs no equipment. Anaglyph is the red/cyan glasses look. RGB-D "
              "sends the depth data itself. Recommended: Half SBS for most 3D displays/headsets."))
        ...
        self.cbo_anaglyph_method.SetToolTip(
            T("Only used when Stereo Format is Anaglyph. The color-filtering recipe for red/cyan glasses "
              "viewing. dubois/dubois2 give the most natural, least color-distorted result for most "
              "red/cyan glasses. gray avoids color distortion entirely but loses color in the image. "
              "Recommended: dubois2."))
        ...
        self.cbo_view_mode.SetToolTip(
            T("Web Streaming: serves the converted 3D video over your network as an MJPEG stream, viewable "
              "from a browser or headset on another device — needed if you're viewing on a separate "
              "headset/device from the one doing the conversion. Local Viewer: displays directly in a "
              "window on this same PC, no network/streaming overhead at all. Recommended: Local Viewer if "
              "you're viewing on the same machine; Web Streaming only if you need to view from elsewhere "
              "on your network."))
        ...
        self.chk_bind_addr.SetToolTip(
            T("What it's for: which network address the streaming server listens on. Unchecked (default) "
              "binds to 127.0.0.1 (this PC only, not reachable from other devices). Check this and set an "
              "address to make the stream reachable from other devices on your network (e.g. a VR "
              "headset).\n"
              "Con: exposing this to your network without Basic Authentication below means anyone on the "
              "same network could potentially view your stream.\n"
        ...
        self.txt_port.SetToolTip(
            T("The network port the streaming server listens on. The device viewing the stream needs to "
              "connect to this same port. Recommended: leave at the default (1303) unless it conflicts "
        ...
        self.cbo_stream_fps.SetToolTip(
            T("Target frame rate for the streamed output. Higher = smoother motion, but more GPU/network "
              "load — actual achieved FPS depends on how fast your GPU can do depth+3D conversion per "
              "frame, this is just the target/cap. Recommended: 30 as a balanced default; try 60 if your "
              "GPU is fast enough to sustain it."))

        self.chk_uncap_fps = wx.CheckBox(self.grp_network, label=T("Uncap FPS"), name="chk_uncap_fps")
        self.chk_uncap_fps.SetValue(False)
        self.chk_uncap_fps.SetToolTip(
            T("What it's for: ignores the Streaming FPS target above and runs as fast as your GPU allows.\n"
              "Con: uses maximum GPU load continuously, and the actual frame rate will vary/be unstable "
              "depending on scene complexity rather than staying at a predictable rate.\n"
              "Recommended: off for a smoother, more predictable experience; on only if you specifically "
              "want maximum possible throughput and don't mind variable frame timing."))
        ...
        self.cbo_stream_height.SetToolTip(
            T("Output height for the streamed video. Lower = less GPU/network load and higher achievable "
              "frame rate, at reduced sharpness. Recommended: 1080 if your GPU/network can sustain it; "
              "720 if you need more headroom for frame rate."))
        ...
        self.cbo_stream_quality.SetToolTip(
            T("JPEG compression quality for each streamed frame (100 = best/largest, 80 = smaller/more "
              "compression artifacts). Lower values reduce network bandwidth needed at some visible "
              "quality cost. Recommended: 90 as a balanced default; lower it if your network connection to "
              "the viewing device is the bottleneck."))
        self.chk_pad_16_9 = wx.CheckBox(self.grp_network, label=T("Padding") + " 16:9", name="chk_pad_16_9")
        self.chk_pad_16_9.SetValue(False)
        self.chk_pad_16_9.SetToolTip(
            T("Pads the streamed frame to a 16:9 aspect ratio (adding black bars where needed) — useful if "
              "your viewing device/headset expects a specific 16:9 input and your captured content isn't "
              "naturally that shape. Recommended: off unless your specific viewer requires 16:9 input."))

        self.chk_gpu_jpeg = wx.CheckBox(self.grp_network, label=T("GPU JPEG"), name="chk_gpu_jpeg")
        self.chk_gpu_jpeg.SetValue(False)
        self.chk_gpu_jpeg.SetToolTip(
            T("What it's for: encodes each streamed frame's JPEG compression on the GPU instead of the "
              "CPU, freeing up CPU time and reducing a bottleneck at high streaming frame rates.\n"
              "Recommended: on if available on your system (grayed out if not supported) and you're "
              "hitting a CPU bottleneck while streaming; otherwise leave off, no downside to defaults."))
        ...
        self.chk_auth.SetToolTip(
            T("What it's for: requires a username/password before anyone can view the stream.\n"
              "How it helps: real security if you've set Address above to something reachable from your "
              "network — without this, anyone who can reach the address/port can view your stream.\n"
              "Recommended: on whenever Address is set to anything other than the local-only default "
              "(127.0.0.1)."))
        ...
        self.cbo_device.SetToolTip(
            T("Which GPU (or CPU) does the AI processing. CPU works without a GPU but is dramatically "
              "slower — not realistic for live real-time capture, only useful for troubleshooting."))
        ...
        self.cbo_screenshot.SetToolTip(
            T("Which method captures your screen each frame. pil: simplest, most compatible, slowest. "
              "mss: faster cross-platform capture. wc_mp (Windows only): uses the modern Windows Graphics "
              "Capture API, generally faster and more reliable than pil/mss on Windows. wc_cuda (Windows + "
              "NVIDIA only): captures directly onto the GPU, skipping a CPU round-trip — the fastest option "
              "when available. Recommended: leave on the automatically-chosen default (the fastest option "
              "your system actually supports) unless you have a specific reason to change it, e.g. "
              "compatibility issues with a particular capture method."))
        ...
        self.cbo_monitor_index.SetToolTip(
            T("Which physical monitor to capture, if you have more than one. Ignored if Window Name below "
              "is set to capture a specific window instead of a whole monitor."))
        ...
        self.cbo_window_name.SetToolTip(
            T("What it's for: capture a SPECIFIC window instead of a whole monitor (e.g. just one game's "
              "window). Leave blank to capture the whole monitor selected above instead.\n"
              "How it helps: lets you convert just one application's content even with other windows open "
              "on screen, and follows the window if it moves.\n"
              "Recommended: use the Reload button to its right to refresh the list of currently open "
              "window titles, then pick the one you want."))
        self.btn_reload_window_name = GenBitmapButton(self.grp_processor, bitmap=load_icon("view-refresh.png"))
        self.btn_reload_window_name.SetToolTip(T("Refresh the Window Name list with currently open windows"))
        ...
        self.txt_crop_top.SetToolTip(
            T("Manually crop this many pixels off the TOP of the captured monitor/window before "
              "conversion — useful for cutting off a title bar, taskbar, or unwanted UI element. 0 = no "
              "crop. See also AutoCrop below for automatic black-bar detection instead of a fixed number."))
        ...
        self.txt_crop_left.SetToolTip(T("Manually crop this many pixels off the LEFT of the capture. 0 = no crop."))
        ...
        self.txt_crop_right.SetToolTip(T("Manually crop this many pixels off the RIGHT of the capture. 0 = no crop."))
        ...
        self.txt_crop_bottom.SetToolTip(T("Manually crop this many pixels off the BOTTOM of the capture. 0 = no crop."))
        ...
        self.cbo_autocrop.SetToolTip(
            T("Automatically detects and removes black bars or plain/flat borders from the capture before "
              "3D conversion, so they don't waste depth detail or get distorted. _TB variants only crop "
              "top/bottom bars; the non-TB variants crop on all sides. Works alongside the manual Crop "
              "Region above — that crops a fixed amount first, this then auto-detects within what's left."))

        self.chk_compile = wx.CheckBox(self.grp_processor, label=T("torch.compile"), name="chk_compile")
        self.chk_compile.SetToolTip(
            T("Optimizes the AI model before running, trading a slower startup (compiles the first time "
              "you start capture after changing settings) for faster per-frame processing afterward — "
              "genuinely valuable here since real-time capture runs the model continuously for as long as "
              "you're streaming. Requires extra setup (see this project's torch_compile docs) to actually "
              "take effect. Recommended: on for a long capture session, once set up."))
        ...
        self.sld_adj_divergence.SetToolTip(
            T("Live nudge for 3D Strength, applied on top of the Stereo Generation panel's own value, "
              "adjustable WHILE capture is actively running — no need to stop and restart to fine-tune "
              "while watching the live result. Use Fine Adjustment (below) for smaller steps."))
        ...
        self.sld_adj_convergence.SetToolTip(
            T("Live nudge for Convergence Plane, adjustable while capture is running, same idea as the 3D "
              "Strength adjustment to its left."))
        ...
        self.sld_adj_foreground_scale.SetToolTip(
            T("Live nudge for Foreground Scale, adjustable while capture is running. Remember this is a "
              "trade-off dial (see the Stereo Generation panel's own Foreground Scale for the full "
        ...
        self.cbo_adj_edge_dilation_y.SetToolTip(
            T("Live nudge for Edge Fix's Y (vertical) value, adjustable while capture is running. Blank = "
              "same as X."))
        self.tgl_adj_fine = wx.ToggleButton(self.grp_adjustment, label=T("Fine Adjustment"),
                                            name="tgl_adj_fine")
        self.tgl_adj_fine.SetToolTip(
            T("Switches the live nudge controls above to smaller step increments, for more precise "
              "fine-tuning instead of coarse jumps."))
        self.btn_adj_align = wx.Button(self.grp_adjustment, label=T("Align to Step"))
        self.btn_adj_align.SetToolTip(
            T("Snaps the current live-adjusted values to the nearest clean step increment, tidying up any "
              "odd in-between value you've landed on while nudging live."))
